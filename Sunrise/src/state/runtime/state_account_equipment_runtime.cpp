...
[[nodiscard]] static bool same_item(const authored_inventory::Item& left,
    ...
    return left.instanceSoid == right.instanceSoid && left.definitionHash == right.definitionHash
           && left.level == right.level && left.levelFraction == right.levelFraction
           && left.quantity == right.quantity && left.flags == right.flags
           && left.seen == right.seen && left.sockets.policy == right.sockets.policy
